...
class CommandProcessor:
    ...
    def __init__(self, model_manager):
        ...
        self.api_manager = APIManager(model_manager)  # Initialize APIManager
    ...
    def process_command(self, command):
        ...
        self.delegate_command(action, detected_topic, command)
    ...
    def identify_action_with_zero_shot(self, command):
        ...
        result = self.classifier(command, possible_actions,clean_up_tokenization_spaces=True)
    ...
    def detect_topic(self, command):
        ...
        result = self.classifier(command, candidate_labels=labels,clean_up_tokenization_spaces=True)
    ...
    def delegate_command(self, action, topic, command):
        ...
        if topic == 'medical' or topic == 'medicine' or topic == 'healthcare':
            ...
            if action == 'summarize':
                ...
                self.medical_processor.summarize_text(text)
            elif action == 'translate':
                text = self.extract_text_from_command(command)
                target_language = self.detect_language(command)
                self.medical_processor.translate_text(text, target_language)
            elif action == 'named_entities':
                text = self.extract_text_from_command(command)
                self.medical_processor.extract_named_entities(text)
            elif action == 'linked_entities':
                text = self.extract_text_from_command(command)
                self.medical_processor.extract_linked_entities(text)
            elif action == 'pos_tagging':
                text = self.extract_text_from_command(command)
                self.medical_processor.pos_tagging(text)
            elif action == 'abbreviations':
                text = self.extract_text_from_command(command)
                self.medical_processor.extract_abbreviations(text)
            elif action == 'hyponyms':
                text = self.extract_text_from_command(command)
                self.medical_processor.extract_hyponyms(text)
            elif action == 'question_generation':
                text = self.extract_text_from_command(command)
                self.medical_processor.generate_questions(text)
            elif action == 'similar_documents':
                query_note, candidate_notes = self.extract_text_for_similarity(command)
                self.medical_processor.find_similar_documents(query_note, candidate_notes)
            else:
                print(f"{Fore.RED}System: Sorry, I don't understand that medical command.")
        else:
            # Use the selected model via APIManager
            if self.model_manager.get_selected_model() == 'ChatGPT':
                self.api_manager.call_chatgpt_api(command)
            elif self.model_manager.get_selected_model() == 'Claude':
                self.api_manager.call_claude_api(command)
            elif self.model_manager.get_selected_model() == 'Gemini':
                self.api_manager.call_gemini_api(command)
            else:
                print(f"{Fore.RED}System: Sorry, I don't understand that command for the general topic.")
